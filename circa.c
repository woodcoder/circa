...
#define URL_FORMAT   "https://carbon-aware-api.azurewebsites.net/emissions/forecasts/current?location=%s&dataEndAt=%s&windowSize=%d"
#define URL_SIZE     256
...
complete within %d minute window.\n", "eastus", 30);
  ...
  int exit_status = errno == ENOENT ? 127 : 126;  // EXIT_ENOENT : EXIT_CANNOT_INVOKE;
  if (errno != 0) {
    fprintf(stderr, "%s\n", strerror(errno));
  }
  return exit_status;
